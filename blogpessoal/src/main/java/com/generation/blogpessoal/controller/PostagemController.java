...
public class PostagemController {
		...
		public ResponseEntity<Postagem> post(@Valid @RequestBody Postagem postagem) { //retornará a requisição/ @valid valida o objeto postagem conforme as regras em "Postagem" (notBlank)/ @RequestBody recebe o objeto postagem e insere no parâmetro Postagem do Método post.
			return ResponseEntity.status(HttpStatus.CREATED) //Executa o Método padrão da Interface JpaRepository e retorna o HTTP 201 se o Objeto foi persistido no Banco de dados.
					.body(postagemRepository.save(postagem));	//INSERT INTO tb_postagens (titulo, texto, data) VALUES ("Título", "Texto", CURRENT_TIMESTAMP())
		}
		...
		public ResponseEntity<Postagem> put(@Valid @RequestBody Postagem postagem) { 
			return postagemRepository.findById(postagem.getId())
					.map(resposta -> ResponseEntity.status(HttpStatus.OK)
						.body(postagemRepository.save(postagem))) //save: se existe o objeto, ele atualiza todos os atributos. Se não existir, ele cria um novo com os dados enviados
